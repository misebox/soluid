...
- `SegmentedControl` picked its tab stop by comparing option objects, so a derived options array matched nothing and left every segment at `tabindex="-1"`, unreachable by keyboard. The stop is chosen by position, so a derived array still matches and two options sharing a value do not both claim it.
- Escape ownership lived in a single list for the whole page, so the newest overlay anywhere answered the key rather than the one focus was in: with a `Dialog` in use and a `Popover` open elsewhere, Escape inside the dialog closed the popover and pulled focus out of the modal onto the popover's trigger. Each overlay now reports both its panel and whatever opened it, so a picker whose focus stays on its trigger still counts as in use, and the overlay the keyboard is inside answers; with focus outside every overlay the newest one still does. Tab used the same page-wide rule, so a popover open elsewhere let Tab out of a modal.
...
- `Slider` left the thumb wherever a drag ended when the parent kept the old value, so the thumb, the filled track and the readout disagreed. Updates made inside an event handler are batched, so the model is compared once it has settled, and only when there is a `value` prop to compare against.
- `createToast` moved to plain timers in the previous release but bound them to no owner, so a pending auto-dismiss, or the removal that follows a manual dismiss, could fire into a store that had already been disposed. A store created inside a component is cleaned up with it; one created at module scope has no owner to hang that on, and ends when the caller says so.
...
- The registry records `VisuallyHidden` as a dependency of `DatePicker`, `TimePicker` and `ColorPicker`, which those components now use to hide the field carrying `name`. Installing them without it left that field visible.
- `confirm` accepted only `y`, so answering `yes` to a question asked as `(y/n)` meant no.
